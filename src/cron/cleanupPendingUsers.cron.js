...
await cleanupQueue.add('cleanupPendingUsersJob', {}, {
...
new redis.Worker('cleanupPendingUsers', async job => {
  ...
  try {
    ...
    console.log(`✅ Nettoyage terminé : ${expiredUsers.length} utilisateurs supprimés.`);

  } catch (err) {
    console.error('❌ Erreur globale dans cleanupPendingUsers via BullMQ :', err);
  }
